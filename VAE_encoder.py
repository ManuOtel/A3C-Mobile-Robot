...
class VAE(nn.Module):
    def __init__(self, z_dim):
        ...
        self.encoder = nn.Sequential(
            # input img N, C ,H ,W
            nn.Conv2d(3, 32, stride=2, kernel_size=3, padding=1, bias=False),  # output N, 32, H/2, W/2
            nn.BatchNorm2d(32),                 # output shape same as input
            nn.LeakyReLU(),    # output shape same as input
            nn.Dropout2d(0.25),                 # output shape same as input ,隨機將整個通道歸零 通道是2D特徵圖 使用伯努利分佈的取樣，每個通道將在每次呼叫forward中以概率p獨立清零。
            #
            nn.Conv2d(32, 64, stride=2, kernel_size=3, padding=1, bias=False),  # output N, 64, H/4, W/4
            nn.BatchNorm2d(64),
            nn.LeakyReLU(),
            nn.Dropout2d(0.25),
            #
            nn.Conv2d(64, 64, stride=2, kernel_size=3, padding=1, bias=False),  # output N, 64, H/8, W/8
            nn.BatchNorm2d(64),
            nn.LeakyReLU(),
            nn.Dropout2d(0.25),
            #
            nn.Conv2d(64, 64, stride=2, kernel_size=3, padding=1, bias=False),  # output N, 64, H/16, W/16
            nn.BatchNorm2d(64),
            nn.LeakyReLU(),
            nn.Dropout2d(0.25),
            #
            nn.Conv2d(64, 64, stride=2, kernel_size=3, padding=1, bias=False),  # output N,64, H/32, W/32
            nn.BatchNorm2d(64),
            nn.LeakyReLU(),
            nn.Dropout2d(0.25),
            #
        ...
        self.z_mean = torch.nn.Linear(512, self.z_dim)     # Mean: 200 node
        self.z_log_var = torch.nn.Linear(512, self.z_dim)  # var:  200 node
        self.z_fmap = torch.nn.Linear(self.z_dim, 512)    # code to feature map
    ...
    def Navigation_forward(self, cur_obs, target_obs):
        ...
        input_state = torch.cat((nxet_fmap, target_fmap), 1)         # Merge into [1, 128]
        # return input_state, recon_pre_obs
    ...
    def reparameterize(self, z_mu, z_log_var):      # 重新random 取參數作為noise ,在乘上 exp 加總 ->code with noise
        ...
        z = z_mu + eps * torch.exp(z_log_var / 2.)  # z_log_var表示 log(variance) 同log(sigma^2),所以 exp(log(var) / 2) 等於std
        return z
