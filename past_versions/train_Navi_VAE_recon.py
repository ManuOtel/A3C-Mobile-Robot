...
class Global_Net(nn.Module):
    def __init__(self, a_dim):
        ...
        self.fpre_a = nn.Linear(3, 64) # action 3 -> 64
    ...
    def choose_act(self, state_fmap, pre_act, last_collide):       # state_fmap:64*2     2022/07/19 add last_collide ##
        ...
        if logits.shape[1] == 3:     ### Action space : 3 ###
            ...
            if last_collide == True:
                ar1 = logits[:, 0:2]
                ar2 = logits[:, 3:5]
                logits = torch.cat((ar1, ar2), dim=0)
                logits = torch.reshape(logits, (1, 4))
                probs = F.softmax(logits, dim=1)
                action = probs.multinomial(1).view(-1)[0].data
                if action == 2:
                    action = torch.tensor(3, device='cuda:0')
                elif action == 3:
                    action = torch.tensor(4, device='cuda:0')
            else:
                probs = F.softmax(logits, dim=1)
                action = probs.multinomial(1).view(-1)[0].data
        ####################################################################################
    ...
    def run(self):
        ...
        while self.g_ep.value < self.MAX_Global_Ep:
            ...
            while True:
                ...
                new_obs, collided, r, done, succeed = self.env.step(action)   # 從環境中 get state, reward ,done or not
...
# Env Setup
    ...
    Self_Adjust_task_step_N = 100               # 自動調整每一個task的最大 step 數  (不是None則預設為True) -> Automatically adjust the maximum number of steps for each task (if not None, the default is True)
...
# a2c hyperparams:
    ...
    GAMMA = 0.9
    Per_Slice_Num = 1                         # 每 N Episode 計算平均 SR -> Calculate the average SR every N Episodes


    num_Worker = 2
    MAX_Global_Ep = 100                       # global Agent (total step 至少 MAX_task_step * Agent_Epoch次)
